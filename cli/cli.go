...
func (cli *CommandLine) StartNode(nodeID, minerAddress string) {
	fmt.Printf("[SUCCESS] starting Node...  %s\n", nodeID)

	if len(minerAddress) > 0 {
		if wallet.ValidateAddress(minerAddress) {
			fmt.Println("[SUCCESS] mining is on | address to receive rewards: ", minerAddress)
		} else {
...
func (cli *CommandLine) reindexUTXO(nodeID string) {
	...
	fmt.Printf("[SUCCESS] done! | there are %d transactions in the UTXO set\n", count)
}
...
func (cli *CommandLine) createWallet(nodeID string) {
	...
	fmt.Printf("[SUCCESS] new address is: %s\n", address)
}
...
func (cli *CommandLine) Run() {
	...
	startNodeMiner := startNodeCmd.String(
		"miner",
		"",
